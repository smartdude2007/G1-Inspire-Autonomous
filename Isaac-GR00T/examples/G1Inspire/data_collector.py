...
class DataCollector(Node):
    def __init__(self):
        ...
        self.motor_state_subscription = self.create_subscription(LowState, '/lowstate', self.motor_state_callback, 10)
        self.motor_cmd_subscription = self.create_subscription(LowCmd, '/lowcmd', self.motor_cmd_callback, 10)
        self.camera_subscription = self.create_subscription(Image, 'camera/image_raw', self.camera_callback, 10)   
        self.lhand_state_subscription = self.create_subscription(MotorStates, '/inspire/state', self.hand_state_callback, 10)
        self.rhand_cmd_subscription = self.create_subscription(MotorCmds, '/inspire/cmd', self.hand_cmd_callback, 10)

        self.control_subscription = self.create_subscription(Bool, '/control', self.control_subscription_callback, 10)

        self.velocity_subscription = self.create_subscription(SportModeCmd, '/sportmodecmd', self.base_velocity_callback, 10)
    ...
    def hand_state_callback(self, msg):
        self.latest_hand_state = np.array([m.q for m in msg.states], dtype=np.float32);
    def hand_cmd_callback(self, msg):
        self.latest_hand_cmd = np.array([m.q for m in msg.cmds], dtype=np.float32);
    ...
    def camera_callback(self, msg):
        if (self.latest_motor_state is None or self.latest_motor_cmd is None or 
            self.latest_hand_state is None or self.latest_hand_cmd is None
            or self.latest_base_velocity is None):
            return
        img = self.bridge.imgmsg_to_cv2(msg, desired_encoding="rgb8")  # (H, W, 3) numpy
        if (self.start_recording):
            latest_observation_state = np.concatenate((self.latest_motor_state, self.latest_hand_state), axis=0)
            latest_action = np.concatenate((self.latest_base_velocity,self.latest_motor_cmd, self.latest_hand_cmd), axis=0)
